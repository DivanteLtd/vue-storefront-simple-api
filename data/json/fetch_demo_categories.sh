...
curl -sS  "https://demo.storefrontcloud.io/api/catalog/vue_storefront_catalog/category/_search?size=2500&from=0" | jq ".hits.hits[]._source | { id, parent_id, name, url_key, is_active, position, level, product_count, children_data: [ .children_data[] | { id, children_data: [ .children_data[] | { id, children_data: [ .children_data[] | { id, children_data: [ .children_data[] | { id } ] } ] } ] } ] }" | jq -s -M \ > categories.json
